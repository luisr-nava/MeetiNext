      ...
      <div className="mt-10 p-5 border border-gray-200">
        {sessions.map((session) => (
          <div key={session.id} className="p-5 shadow-xs flex items-center">
            <div className="lg:flex lg:gap-2 items-center flex-1">
              <p>{formatUserAgent(session.userAgent!)}</p>
              {isCurrentDevice(session.id) && (
                <p className="text-green-600 font-bold border bg-green-200 border-green-200 rounded-sm inline-block px-3 py-1 uppercase text-xs">
                  Este Dispositivo
                </p>
              )}
            </div>
